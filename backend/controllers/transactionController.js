...
class TransactionController {
    constructor() {
        this.transactions = [
            { id: 1, amount: 500, type: 'income', description: 'Depósito inicial', date: new Date(), userId: 1, walletId: 1, createdAt: new Date(), updatedAt: new Date() },
            { id: 2, amount: 1000, type: 'expense', description: 'Compra de ações', date: new Date(), userId: 1, walletId: 1, createdAt: new Date(), updatedAt: new Date() }
        ];
    ...
    async create(req, res) {
        if (process.env.USE_DB !== 'true') {
            const { amount, type, description, date, userId, walletId } = req.body || {};
            if (amount === undefined || !type || !userId) return res.status(400).json({ error: 'amount, type e userId são obrigatórios' });
            const id = this.transactions.length ? Math.max(...this.transactions.map(t => t.id)) + 1 : 1;
            const newItem = { id, amount, type, description: description ?? '', date: date ? new Date(date) : new Date(), userId, walletId: walletId ?? null, createdAt: new Date(), updatedAt: new Date() };
    ...
    async update(req, res) {
        if (process.env.USE_DB !== 'true') {
            ...
            if (date !== undefined) this.transactions[idx].date = new Date(date);
